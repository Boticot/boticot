    ...
    def __enter__(self):
        ...
        for agent in all_trained_agents:
            try :
                ...
                AgentsService.get_instance().load_agent(agent.get("name"), agent.get("lastVersion"))
            except Exception as e:
                logger.error("Exception when loading agent {0} with version {1} inside Cron Loader. {2}".format(agent.get("name"), agent.get("lastVersion"), e), exc_info=True)
