...
class TestResponsesService:
    ...
    def test_should_return_something_when_getting_agent_responses_by_intent(self, mocker):
        mocker.patch("responses_service.ResponsesRepository.find_agent_responses_by_intent", return_value=self.db_responses)
        assert responses_service.ResponsesService.get_instance().get_agent_responses_by_intent("test1", "intent_1") == self.db_responses_get_agent_responses

    def test_should_return_something_when_getting_response(self, mocker):
        mocker.patch("responses_service.ResponsesService.get_agent_responses_by_intent", return_value=self.db_responses_get_agent_responses)
        assert responses_service.ResponsesService.get_instance().get_response("test1", "intent_1") == self.db_responses_get_response
